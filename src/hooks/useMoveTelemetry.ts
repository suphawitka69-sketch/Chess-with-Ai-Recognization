 ...
 * ---------------------------------------------------------------------------
 * เก็บค่า telemetry แบบเรียลไทม์ "ระหว่างตา" ที่ผู้เล่นกำลังคิดอยู่
 * อัปเดตรองรับ triggerFlags และ alias selector ให้เชื่อมกับ PanicMeter + useMoveTelemetry
 * ---------------------------------------------------------------------------
 */
...
export interface TelemetryState {
  /** Panic Score สด 0-100 ที่ผ่าน EMA smoothing มาจาก PanicCalculator แล้ว */
  readonly currentPanicScore: number;
  /** ส่วนต่างจาก panicScore ของตาก่อนหน้าที่ commit ไปแล้ว (บวก = ลนขึ้น, ลบ = สงบลง) */
  readonly panicDelta: number;
  /** 0-1 ผสม cancel + repeat + idle ตามสูตรใน Schema 1 (behavior.hesitationIndex) */
  readonly hesitationIndex: number;
  /** true เมื่อเวลาคงเหลือของฝ่าย active ต่ำกว่า TIME_PRESSURE_RATIO_THRESHOLD ของ base time */
  readonly isUnderTimePressure: boolean;
  /** แฟล็กกระตุ้นความลนที่ตรวจพบ (สำหรับนำไปแสดง Badge บน PanicMeter) */
  readonly triggerFlags: readonly string[];
  /** เก็บไว้ภายในเพื่อคำนวณ panicDelta ของตาถัดไป */
...
/** alias รองรับทั้งสองชื่อ เพื่อกันคอมโพเนนต์อื่นเรียกผิด */
